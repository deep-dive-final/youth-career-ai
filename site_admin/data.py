...
# E5 모델 임베딩 생성 함수
def get_Embedding_e5 (original_text_list):
    print("[get_Embedding_e5] start ")

    # E5 모델의 특성에 맞게 접두사 추가: 'passage' (저장용) 또는 'query' (검색용)
    prefixed_texts = [f"passage: {text}" for text in original_text_list]
...
# Gemini 임베딩 생성 함수
def get_Embedding_gemini(original_text_list):
    ...
    result = genai.embed_content(
        ...
        task_type="document",       # 저장용은 document, 검색용은 query 사용
        title="Policy Data"         # 선택 사항: 문서의 제목을 명시하면 품질이 좋아짐
    )
...
{item.get('addAplyQlfcCndCn', '')} ".strip() for item in api_list]
    ...
    embedding_e5 = get_Embedding_e5(original_text_list)
    embedding_gemini = get_Embedding_gemini(original_text_list)
    ...
    for i, item in enumerate(api_list):
        ...
        vector_doc = {
                ...
                "metadata":{
                    ...
                    "plcyExplnCn":item.get('plcyExplnCn', ''),
                    "category":item.get('lclsfNm', ''),
                    "sub_category":item.get('mclsfNm', ''),
                    "plcySprtCn":item.get('plcySprtCn', '')
                },
                "inserted_at": current_time
            }
